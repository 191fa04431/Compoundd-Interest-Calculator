# -*- coding: utf-8 -*-
"""
Created on Thu May 23 15:47:28 2024

@author: ch.avinash chowdary
"""
...
def compound_int(Principal, Int_Rate, No_Of_Years):
    ...
    comp=1.0
    for i in range(0, int(No_Of_Years.iloc[0])):
        comp=comp*(1+Int_Rate/100)
        #st.write(comp)
    comp=float(Principal.iloc[0])*(comp-1)
    comp_text= str("Compound Interest is " + str("%.3f" % comp.iloc[0]) )
    st.write(comp_text)
